...
## [your code here]
filtered_sorted_df <- df %>% filter(Grad.Rate >= 80 & Grad.Rate <= 99) %>%  arrange(desc(Grad.Rate))
filtered_sorted_df
hw1lab1 <- select(filtered_sorted_df,X,Grad.Rate)
hw1lab1
...
# Find max and min Outstate tuition for df
df_summary <- df %>%
  group_by(Private) %>%
  summarise(
    max = max(Outstate, na.rm = TRUE),
    min = min(Outstate, na.rm = TRUE)
  )

print(df_summary)

# Find max and min Outstate tuition for college
